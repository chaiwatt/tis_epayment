...
@if(auth()->user()->SetRolesLicenseCertify() == "true" ||  in_array("29",auth()->user()->RoleListId))         
                     @if($certi_cb->status >= 6 && !is_null($certi_cb->CertiCBCostTo))
                                ...
                                @endphp
                            <a  class="btn {{$cost_btn}}" href="{{  url('certify/estimated_cost-cb/'.$Cost->id.'/edit') }}" >
 ...
 @endphp
   <div class="btn-group">
    ...
    @else 
        <a  class="btn btn-warning" href="{{ url("certify/auditor-cb")}}" >
            แต่งตั้งคณะฯ
        </a>
    @endif
@endif 
...
@endforeach
    ...
    @else 
         <a  class="btn btn-warning" href="{{ url("certify/save_assessment-cb")}}" >
            ผลการตรวจประเมิน
        </a>
    @endif
                      ...
                      @if($certi_cb->status >= 17)
                        ...
                        @include ('certify/cb/check_certificate_cb/modal.modalstatus22',['file_all'=> $certi_cb->CertiCBFileAlls,
                          ...
                          @if(!is_null($certi_cb->CertiCBExportTo))
                            ...
                            @else 
                            <a  class="btn btn-warning" href="{{  url('certify/certificate-export-cb') }}" >
                               ออกใบรับรอง
                            </a>
                            @endif 
                   @endif
