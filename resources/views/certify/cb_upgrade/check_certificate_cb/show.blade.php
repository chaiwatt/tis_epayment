...
<div class="row">
    <div class="col-sm-12 ">
                ...
                @if(auth()->user()->SetRolesLicenseCertify() == "true" ||  in_array("29",auth()->user()->RoleListId))  
                    ...
                    <!-- START  status 9 แต่งตั้งคณะตรวจเอกสาร-->  
                    @if($certi_cb->status >= 9)

                        @if(count($certi_cb->CertiCBAuditorsManyByReview) > 0) 
                            @php 
                                $auditors_btn =  '';
                                $auditors_icon =  '';
                            if($certi_cb->CertiCBAuditorsDocReviewStatus == "StatusSent"){
                                $auditors_btn = 'btn-success';
                                $auditors_icon =  '<i class="fa fa-file-text"></i>';
                            }elseif($certi_cb->CertiCBAuditorsDocReviewStatus == "StatusNotView"){
                                $auditors_btn =  'btn-danger';
                                $auditors_icon =  '<i class="fa fa-arrow-circle-right"></i>';
                            }elseif($certi_cb->CertiCBAuditorsDocReviewStatus == "StatusView"){
                                if ($certi_cb->more_doc_require == 2) {
                                    $auditors_btn = 'btn-info';
                                }else{
                                    $auditors_btn = 'btn-primary';
                                }
...
{{-- {{$auditors_btn}} --}}
                                <button type="button" class="btn {{$auditors_btn}} dropdown-toggle" data-toggle="dropdown">
                                    แต่งตั้งคณะตรวจเอกสาร  <span class="caret"></span>
                                </button>
                                        ...
                                        @if($certi_cb->status == 10)   <!-- อยู่ระหว่างดำเนินการ -->
                                                <form action="{{ url('/certify/auditor-cb/create')}}" method="POST" style="display:inline" > 
                                                    ...
                                                    <button class="btn btn-warning" type="submit"   style="width:750px;text-align: left"> 
                                                        <i class="fa fa-plus"></i>    แต่งตั้งคณะตรวจเอกสาร
                                                    </button>
                                                </form>
                                        @endif

                                    @php $i_key = 0;   @endphp
                                    @foreach($certi_cb->CertiCBAuditorsManyByReview as $key => $item)
                                        @php 
                                            $auditors_btn =  '';
                                        ...
                                        @if ($item->status_cancel != 1)
                                            <a  class="btn {{$auditors_btn}} " href="{{ url("certify/auditor_cb_doc_review/auditor_cb_doc_review_edit/".$item->id)}}" style="background-color:{{$auditors_btn}};width:750px;text-align: left">
                                                ครั้งที่ {{ ($i_key + 1 )}} :  
                                                {{ $item->auditor ?? '-'}}
                                            </a> 
                                            <br>
                        ...
                        @else 
                            {{-- <a class="form_group btn btn-warning" href="{{ url("certify/auditor-cb")}}" >
                                แต่งตั้งคณะฯ
                            </a> --}}
                            ...
                            {{-- <div class="btn-group form_group"> --}}
                                {{-- <form action="{{ url('/certify/auditor-cb/create')}}" method="POST" style="display:inline" > 
                                    ...
                                    <button class="btn btn-warning" type="submit" >
                                        <i class="fa fa-plus"></i>    แต่งตั้งคณะฯ fsdf
                                    </button>
                                </form> --}}


                                <a  class="form_group btn btn-warning" href="{{route('auditor_cb_doc_review',['id' => $selectedCertiCb])}}" >
                                    <i class="fa fa-file-text-o" aria-hidden="true"></i>  แต่งตั้งคณะตรวจเอกสาร
                                </a>
                            {{-- </div> --}}
                    ...
                    <!-- START  status 9 แต่งตั้งคณะฯ-->  
                    @if($certi_cb->status >= 9 && $certi_cb->certiCBAuditorsDocReviewStatusFinish->count() > 0 && $certi_cb->more_doc_require == 2)
                ...
                @if(auth()->user()->SetRolesLicenseCertify() == "true" ||  in_array("26",auth()->user()->RoleListId))  
                    ...
                    @if(count($certi_cb->CertiCBPayInOneMany) > 0 && $certi_cb->certiCBAuditorsStatusFinish->count() > 0)
                        @php 
                            $payin1_btn =  '';
                            $payin1_icon =  '';
                ...
                @if((auth()->user()->SetRolesLicenseCertify() == "true" ||  in_array("29",auth()->user()->RoleListId)) && count($certi_cb->CertiCBPayInOneStatusMany)  > 0 )  
                    {{-- {{$certi_cb->CertiCBSaveAssessmentMany->count()}} --}}
                    ...
                    <a  class="form_group btn btn-warning" href="{{ url("certify/check_certificate-cb/create_team_review/".$certi_cb->id)}}" >
                        ต้องตั้งคณะทบทวนนะ
                    </a>
