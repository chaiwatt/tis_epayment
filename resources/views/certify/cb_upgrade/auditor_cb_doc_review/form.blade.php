    ...
    <div class="col-md-12">
        <div class="col-md-9">
            <div class="form-group {{ $errors->has('certi_no') ? 'has-error' : ''}}">
                ...
                <div class="col-md-7">
                    @if(isset($app_no))
                        ...
                        [$certiCb->id => $certiCb->app_no], // กำหนดค่าเดียวที่ต้องการแสดง
                        null, // ไม่เลือกค่าใดเลยโดยเริ่มต้น
                        ['class' => 'form-control',
                        'id' => 'app_certi_cb_id',
                        'placeholder'=>'- เลขคำขอ -', 
                        'required' => true]) !!}
            ...
            <div class="form-group {{ $errors->has('no') ? 'has-error' : ''}}">
                ...
                <div class="col-md-7">
                    {!! Form::text('no', $certiCb->name, ['class' => 'form-control', 'placeholder'=>'', 'required' => true]); !!}
                </div>
            </div>
            <div class="form-group {{ $errors->has('no') ? 'has-error' : ''}}">
            ...
            @foreach($auditors_status as $key => $item)    
                <tr class="repeater-item">
                    ...
                    <td class="text-center text-top">
                        <div class="form-group {{ $errors->has('taxid') ? 'has-error' : ''}}">
                            <div class="col-md-9">
                                <select name="list[status][]" class="form-control item status select2" data-name="status" required>
                                    ...
                                    @foreach (App\Models\Bcertify\StatusAuditor::pluck('title', 'id') as $id => $title)
                                        <option value="{{ $id }}" {{ isset($item->status) && $item->status == $id ? 'selected' : '' }}>
                                            {{ $title }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <td class="align-right text-top ">
                        <div class="td-users">
                            @if(count($item->CertiCBAuditorsLists) > 0)
                                @foreach($item->CertiCBAuditorsLists as $key1 => $item1)
                                    <input type="text" name="filter_search" value="{{ $item1->temp_users ?? '' }}" class="form-control item" readonly>
                                    <input type="hidden" name="list[temp_users][{{$item->status}}][]" value="{{$item1->temp_users}}">
                                    <input type="hidden" name="list[user_id][{{$item->status}}][]" value="{{$item1->user_id}}">
                                    <input type="hidden" name="list[temp_departments][{{$item->status}}][]" value="{{$item1->temp_departments}}">
                                @endforeach
                            @else
                                <input type="text" name="filter_search" value="" class="form-control item" placeholder="" data-name="filter_search" required>
                            @endif
                        </div>
                        <div class="div-users"></div>
                    </td>
                    ...
                    <td class="align-top text-top  ">
                        ...
                        <div class="td-departments">
                            @if(count($item->CertiCBAuditorsLists) > 0)
                                ...
                                @endforeach
                            @else
                                <input type="text" name="department" class="form-control item" 
                                       @if('' == 'required') 
                                           required="required" 
                                       @else 
                                           readonly="readonly" 
